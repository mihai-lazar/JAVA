...
public class assistenteGui5 {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					assistenteGui5 window = new assistenteGui5();
	...
	private void initialize() {
		...
		eliminarEventoAgenda.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (seleccionIndex >= 0) {
					if(seleccionIndex == 0)
						if(usuarioActual.aviso == 1) {	
							usuarioActual.aviso = 0;
							usuariosNombresListaDefault.set(usuariosLista.indexOf(usuarioActual), usuarioActual.nombre);
							usuariosNombresLista.setModel(usuariosNombresListaDefault);          // seteamos el modeo de la lista visible a la lista defailt
						}
				...
				if (nombreUsuarioActivo.getText() != "") {
					if(estadoUsuario.isSelected())                                     
						...
						if(usuarioActual.feedOcupado.size() > 0){ 
							...
							usuarioActual.feedOcupado = new ArrayList<Object>();
	...
	private class Usuario {                                                            //Clase usuario
		...
		List<Object> feedDesoupado = new ArrayList<Object>();
		List<Object> feedOcupado = new ArrayList<Object>();
	...
	private class feedTimer extends TimerTask {                                        //Clase Timer
	    public void run() {	    	
	    	for(int i = 0; i < usuariosLista.size(); i ++) {
	    		...
	    		for(int e = 0; e < usuarioActualizacionFeed.intereses.size(); e ++) {
	    		    if(usuarioActualizacionFeed.intereses.get(e) == 1)  {
	    		    	...
	    		    	if(usuarioActualizacionFeed.estado == 0) {
	    		    		usuarioActualizacionFeed.feedDesoupado.add(intereses.get(e).getText());
	    		    		if (nombreUsuarioActivo.getText() != "") {
	    		    		...
	    		    		usuarioActualizacionFeed.feedOcupado.add(intereses.get(e).getText());	    		    		
	    		    	}
	    		    }
	    		}
	    	}
	...
	private class eventosTimer extends TimerTask {                                      //Clase Timer
	    public void run() {	
	     	
	    	int cambio = 0;
			for(int i = 0 ; i < usuariosLista.size() ; i++)  {                          // removimos el usuario de la lista de usuarios
				Usuario usuarioEventoTimer = (Usuario) usuariosLista.get(i);
				if(usuarioEventoTimer.agenda.size() > 0) {
					...
					Instant instantFechaHoraActualMas = instantFechaHoraActual.plus(Duration.ofMinutes(30));
				
					Evento eventoAgenda = (Evento) usuarioEventoTimer.agenda.get(0);					
					...
					if(instantFechaHoraActualMas.isAfter(instantFechaHoraAgenda)) {
						...
						if(usuarioEventoTimer.aviso == 0) {
							usuarioEventoTimer.aviso = 1;
							usuariosNombresListaDefault.set(i, usuarioEventoTimer.nombre + " EVENTO PROXIMO");
							cambio = 1;
						}
						
						Duration duration = Duration.between(instantFechaHoraActual, instantFechaHoraAgenda);	
						if(duration.toMinutes() == 0) {
							usuarioEventoTimer.agenda.remove(0);
							usuarioEventoTimer.aviso = 0;
							usuariosNombresListaDefault.set(i, usuarioEventoTimer.nombre);
							
							if (nombreUsuarioActivo.getText() != "") {
								if(usuarioEventoTimer.equals(usuarioActual)) {
									agendaEventosListaDefault.removeElementAt(0);                // se elimina el elemento de la lista
									agendaEventosLista.setModel(agendaEventosListaDefault);      //ponemos el modelo	
								}
							}
