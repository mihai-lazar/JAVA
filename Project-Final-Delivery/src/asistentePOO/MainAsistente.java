...
public class MainAsistente {
	...
	private void initialize() {
		...
		FeedTimer.schedule(new TimerFeedLlamadas(), 0, 10000); 							 //Creacion de los parametros del timer
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (seleccionIndex >= 0) {
							...
							for (int i = 0; i < usuarioActual.getFeedDesocupado().getLargo(); i++) {               // revisamos todos los feed que tiene el usuario en su lista feed
								if(usuarioActual.getFeedDesocupado().getNoticia(i).getEstado() == EstadoNoticiaUsuario.VISTA) {
									feedListaDefault.addElement("<html><font color=red>" + usuarioActual.getFeedDesocupado().getNoticia(i).getTitulo() + "</font></html>"); 
								}else {
